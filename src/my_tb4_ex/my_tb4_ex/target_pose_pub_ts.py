...
class TargetPosePublisher(Node):
    def __init__(self):
        ...
        self.rgb_sub_test = self.create_subscription(CompressedImage, 'oakd/rgb/image_raw/compressed', self.rgb_test_callback, 10)
        self.depth_sub_test = self.create_subscription(Image, 'oakd/stereo/image_raw', self.depth_test_callback, 10)
        # self.rgb_sub = Subscriber(self, CompressedImage, 'oakd/rgb/image_raw/compressed')
    ...
    def rgb_test_callback(self, msg):
        self.info('Received RGB image for testing.')
    
    def depth_test_callback(self, msg):
        self.info('Received depth image for testing.')
    ...
    def image_callback(self, rgb_msg, depth_msg):
        ...
        results = self.model(rgb_img, conf=CONFIDENCE, verbose=False)
